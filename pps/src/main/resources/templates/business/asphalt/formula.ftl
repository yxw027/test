                ...
                <div class="row container-main" style="overflow-y: auto;overflow-x: hidden;margin-bottom: 20px;">
                    <div class="col-lg-12" style="height: 50px;margin-bottom: 1px;">
                        <div class="card-box" style="height: 50px;padding: 0;padding-left: 15px;">
                            ...
                            <@shiro.hasPermission name="asphaltFormula:insert">
            ...
            <div class="row col-lg-12" style="margin-top: 5px;">
                ...
                <div class="col-lg-4">
                    <table style="width: 100%;height: 300px;margin-left: 15px;">
                        ...
                        <tr>
                            <td class="text-muted">国标：</td>
                            <td>#:getStandardLevel(standardLevel)#</td>
                        </tr>
                        <tr>
                        ...
                        <tr>
                            <td class="text-muted">沥青温度：</td>
                            <td>
                                #if(asphaltTemp && asphaltTemp.formulaValue!=undefined &&
                                asphaltTemp.firstLow!=undefined &&
                                asphaltTemp.firstHigh!=undefined){#
                                #:asphaltTemp.formulaValue# ℃(#:asphaltTemp.firstLow#℃ ~ #:asphaltTemp.firstHigh#℃)
                                #}#
                            </td>
                        </tr>
                        <tr>
                            <td class="text-muted">油石比：</td>
                            <td>
                                #if(oilStone && oilStone.formulaValue!=undefined && oilStone.firstLow!=undefined &&
                                oilStone.firstHigh!=undefined){#
                                #:oilStone.formulaValue# %(#:oilStone.firstLow#% ~ #:oilStone.firstHigh#%)
                                #}#
                            </td>
                        </tr>
                        <tr>
...
<script>
    _org_tree_view.bind({
        select: function (e) {
            window.location.href = "asphaltFormula/page";
        }
    });
    var layer = layui.layer;
    var back_asphaltId = ${asphaltId};
    ...
    $(function () {
        ...
        $("#addNew").on('click', function () {
            window.location.href = "asphaltFormula/formulaDetail?asphaltId=" + _asphalt_select_asphaltId + "&bidId=" + _asphalt_select_orgId;
        });
        ...
        $('body').on('click', '.formula_edit', function () {
            ...
            window.location.href = "asphaltFormula/formulaDetail?dataId=" + id + "&bidId=" + _asphalt_select_orgId + "&asphaltId=" + _asphalt_select_asphaltId;
        })
        /**
         * 查看
         */
        $('body').on('click', '.formula_search', function () {
            var id = $(this).attr("data-id");
            window.location.href = "asphaltFormula/formulaDetail?dataId=" + id + "&bidId=" + _asphalt_select_orgId + "&asphaltId=" + _asphalt_select_asphaltId + "&readyOnly=readyOnly";
        })
        ...
        $('body').on('click', '.formula_delete', function () {
            ...
            layer.confirm("确认删除配方" + name + "吗？", function () {
                httpUtils.ajax("asphaltFormula/delete", JSON.stringify({id: id}), function () {
        ...
        $('body').on('click', '.grading_setting', function () {
            var id = $(this).attr("data-id");
            httpUtils.ajax("asphaltFormula/getAsphaltInfoByFormulaId", {formulaId: id}, function (data) {
                data.formulaId = id;
                openWindows(data);
            });
        })
        ...
        $('body').on('click', '#addSieve', function () {
            ...
            httpUtils.ajax("asphaltSieveSetting/addList", JSON.stringify(arr), function (data) {
                if (data.success) {
        ...
        $("body").on('click', ".processed_data", function (res) {
            ...
            httpUtils.ajax("asphaltMetadata/processData", {formulaId: id}, function (data) {
                layer.closeAll();
    ...
    function initList(id) {
        httpUtils.ajax("asphaltFormula/showAll?asphaltId=" + id, function (data) {
    ...
    function renderingList(data) {
        ...
        for (var i = 0; i < data.length; i++) {
            if (data[i].asphaltTemp == undefined) {
                data[i].asphaltTemp = null
            }
    ...
    function initChart(data) {
        ...
        for (var i = 0; i < data.sieves.length; i++) {
            // target.push([parseFloat(sieveLevelArray[i]),parseFloat(data.sieves[i].formulaValue)]);
            // min.push([parseFloat(sieveLevelArray[i]),parseFloat(data.sieves[i].firstLow)]);
            // max.push([parseFloat(sieveLevelArray[i]),parseFloat(data.sieves[i].firstHigh)]);
        ...
        option = {
            ...
            tooltip: {
                ...
                // formatter:function (e) {
                //     return '筛孔'+e[0].value[0]+'mm通过率<br/> <div style="color:#f42024">'+e[0].seriesName+':'+e[0].value[1]+'%<div/> <div style="color:#1fd41f">'+e[1].seriesName+':'+e[1].value[1]+'%</div> <div style="color:#f42024">'+e[2].seriesName+':'+e[2].value[1]+'%</div>';
                // }
            },
            color: ["#f42024", "#07ff7c"],
            legend: {
...
<script id="sieveTemplate" type="text/x-kendo-template">
    <div id="sieveWindows" class="col-lg-12">
        <table id="sieveTable" border="1" cellspacing="0">
            ...
            <tr id="sieveTitle">
                ...
                # } #
                # for(var i=0;i< info.powderNum ; i++){ #
                # var index = i+info.aggregateNum; #
                # var fName = names[index] #
            ...
            <tr id="fm_#:i#" style="height: 38px;">
                ...
                # } #
                # for(var j=0;j< info.powderNum ; j++){ #
